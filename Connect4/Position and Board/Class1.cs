...
namespace Connect4
{
	public class GamePosition
	{
		...
		public void PlayMove(int columnIndex)
		{
			...
			int rowIndex = (int)((_bitBoard >> 3 * columnIndex) & 0b111);  // Next row index is equal to the current column height
			if (toMove == CheckerStateEnum.Yellow)
				...
				_bitBoard += ((ulong)1) << (7 * columnIndex) + rowIndex + 21; // The first 21 bits are reserved for the column heights
			_bitBoard += ((ulong)1) << 3 * columnIndex;  // Increasing the column height by one.
		...
		public void TakebackMoves(int numMoves)
		{
			...
			for (int i = 0; i < numMoves; i++)
			{
				...
				int rowIndex = (int)((_bitBoard >> 3 * columnIndex) & 0b111) - 1;
				if (PlayerToMove == CheckerStateEnum.Yellow)
					_bitBoard -= ((ulong)1) << (7 * columnIndex) + rowIndex + 20;
				_bitBoard -= ((ulong)1) << 3 * columnIndex;  // Decreasing the column height by one.
			...
			foreach (int[] dir in _SearchDirections)
			{
				foreach (int magnitude in new int[] { 1, -1 })    //We must search in both directions along the line defined by the vector dir
				{
						 ...
						 && GetPositionState(nextRow, nextColumn) == whoMoved)
					{
						counter++;
						if (counter == 3)
							return CheckerStateEnum.Empty; //return whoMoved;  // We have ourselves a winner!
